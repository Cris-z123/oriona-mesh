...
class ModelGatewayService:
    ...
    def call_stream(self, call: ModelCall) -> Iterator[GenerationDelta]:
        ...
        while True:
            try:
                ...
                first = self._first_chunk(chunks, first_token_timeout)
                break
            except GatewayError as exc:
                retryable = exc.error_class in _RETRYABLE_CLASSES and attempt < retries
                if not retryable:
        ...
        yield from self._stream_rest(
            chunks, first, call, model, sanitized, started, total_timeout, attempt
        )
    ...
    def _first_chunk(self, chunks: Iterator[str], timeout: float) -> str | None:
    ...
    def _build_sanitized(
        ...
        return SanitizedModelCall(
            ...
            timeout_seconds=float(options.get("timeout_seconds", 0) or 0),
