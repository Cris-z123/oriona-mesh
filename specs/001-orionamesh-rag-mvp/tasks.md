...
- [x] T139 修复阶段 8.1 审查回归：登出刷新后以新整组令牌撤销后继会话、认证上下文按会话身份隔离并同步资料更新；`ContextRail` 单节点响应式迁移；`ErrorState` 提供可访问的 trace_id 复制入口（复制反馈 2 秒复位）；认证/资料表单采用 React Hook Form + Zod（`zodResolver`）本地校验；首页服务端重定向；查询键集中维护于 `@/lib/query-keys`（删除 feature 重导出）；`ErrorState` 为唯一错误呈现组件（`ApiErrorNotice` 删除）；`DocumentStatusFilter` 类型归属 `lib/api/types.ts`；pnpm 10/11 构建脚本白名单兼容，并新增对应回归测试于 `frontend/src/`、`frontend/tests/`、`pnpm-workspace.yaml`（67 passed，lint/typecheck/build 全绿）
...
**目标**：消除资料任务因 Celery 载荷类型差异在尝试开始前无限失败的阻塞；使部署健康实际覆盖 worker
控制通道；保证新会话和被拒绝消息的客户端临时状态及时与服务端真相一致。

### 先写失败测试

- [x] T163 [P] 为 Celery 投递边界和全部资料 worker 包装器编写回归测试：UUID 投递必须序列化为字符串，
      包装器收到字符串或 UUID 都只规范化一次并进入处理函数；于 `backend/tests/unit/workers/`
- [x] T164 [P] 为首条消息创建后的会话侧栏刷新、服务端未接受消息时撤销乐观消息并恢复输入编写 Vitest
      组件回归测试，于 `frontend/tests/component/`
- [x] T165 [P] 为 worker control ping 健康检查、部署等待与完整 Compose 冒烟的静态/运行契约编写测试，
      于 `backend/tests/integration/test_delivery_stack.py`

### 实现与验证

- [x] T166 实现 Celery 跨进程 UUID 载荷正规化及六个资料 worker 的兼容入口，不改变 PostgreSQL 任务
      状态机、阶段编排或恢复扫描器；于 `backend/app/workers/`
- [x] T167 实现会话创建查询失效和未接受消息的乐观状态回滚；于
      `frontend/src/features/conversations/`
- [x] T168 为 worker 添加 Celery control ping 健康检查，并使 Release 部署/冒烟等待该检查；同步运行
      手册的 worker 验证命令，于 `deploy/compose/compose.yaml`、
      `backend/tests/integration/test_delivery_stack.py`、`specs/001-orionamesh-rag-mvp/quickstart.md`
- [x] T169 运行受影响后端 pytest、前端 Vitest、静态质量检查及（可用时）完整 Compose 冒烟；将实际结果
      写入 `specs/001-orionamesh-rag-mvp/quickstart.md`

---

## 阶段 16：全局会话历史恢复

**目标**：将“单会话回答只绑定一个知识库”与“浏览本人全部历史不需预选知识库”明确分离。先完成后端
列表契约和查询实现，再让左侧侧栏展示带知识库名称的全局历史；不得将会话历史放入中央内容区，也不得
扩大检索或会话授权范围。

**独立测试**：在多个知识库各有会话时，未选择知识库进入对话页仍可分页浏览全部历史；每行显示正确
知识库名称；点击任一会话直接恢复其绑定 URL 上下文；新建、重命名或删除后全局排序、total 与当前选择
保持一致。

### 先写失败测试

- [x] T170 [P] [US4] 为 `GET /conversations` 的全局分页、可选 `knowledge_base_id` 筛选、最近活动排序、`knowledge_base_name` 授权投影及跨用户隔离编写后端契约/集成失败测试，于 `backend/tests/contract/`、`backend/tests/integration/`
- [x] T171 [P] [US4] 为未选择知识库仍展示全局历史、会话行知识库标签与空标题默认文案、跨知识库历史点击恢复 URL、全局分页与 mutation 后刷新编写 Vitest 组件失败测试，于 `frontend/tests/component/core-conversation-workflow.test.tsx`

### 后端契约与实现

- [x] T172 [US4] 将会话列表查询实现为当前用户范围内的全局分页，并支持可选知识库筛选；以同一已授权关联返回 `knowledge_base_name`，同步 Pydantic、仓储、服务、路由和 OpenAPI，且不向 `conversations` 新增持久化冗余字段，于 `backend/app/`、`specs/001-orionamesh-rag-mvp/contracts/openapi.yaml`、`specs/001-orionamesh-rag-mvp/data-model.md`

### 前端实现与验证

- [x] T173 [US4] 仅在 T170/T172 契约通过后重构 `ConversationSidebar`、会话查询键与 URL 恢复：默认消费无筛选全局历史、展示知识库名称、点击行原子恢复会话和知识库上下文，且新建/重命名/删除精确刷新全局历史，于 `frontend/src/features/conversations/`、`frontend/src/lib/api/`、`frontend/src/components/app-shell/`
- [x] T174 [US4] 运行 T170/T171 覆盖的后端契约/集成测试、受影响 Vitest、前端 lint/format/类型检查与 OpenAPI 校验；记录全局历史、跨知识库恢复和未选择知识库新建路径的验证结果，于 `specs/001-orionamesh-rag-mvp/quickstart.md`
...
- 前端用户故事沿用后端顺序，以避免消费未冻结 API；阶段 8、8.1、9、10、12、13 的 `[P]` 测试可与不同组件实现并行。阶段 12 的 T145/T146 完成后，T147～T151 可按文件边界并行，最后执行 T152；阶段 13 必须先完成 T153～T156，再按 T157～T160 实现，最后执行 T161。阶段 16 必须先完成 T170/T171，再完成 T172；T173 依赖 T172 冻结的契约，最后执行 T174。
...
- 前端仅在 T105 后：US1 的 T106/T107、共享 UI 的 T134/T135、US3 的 T118 可先独立建立失败测试；US2 的 T113 依赖 T134–T138；核心流程体验修复以 T140～T144 先写测试并淘汰 E2E，T145/T146 是其实现前置。阶段 13 以 T153～T156 先写失败测试，再进入 T157～T160；阶段 16 以 T170/T171 先写契约与组件测试，T172 后才可实现 T173；不引入 E2E。
