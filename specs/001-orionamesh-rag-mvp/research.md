...
## 前端体验一致性修复（2026-08-22）

**决定**：全局工作区侧栏保留品牌、导航和账户入口，并在对话路由且 URL 已明确选择知识库时承载
会话历史；历史绝不进入中央主内容区。首次进入未选择知识库的对话页不回退到任意有效知识库；资料页标题
必须显示所属知识库，上传接受后立即呈现真实的非终态资料行。

**理由**：全局展示按“第一个有效知识库”推断出的会话历史，会让资料管理与对话范围失配。个人 RAG
的首要认知负担是确认“我正在基于哪份资料工作”；把会话历史限制在明确的对话上下文，能消除错误
范围和重复信息，同时不牺牲侧栏快速恢复历史的效率。
