...
### 阶段 17 Evidence Console UI/UX 验收计划（T179，待实施）

- **范围**：保留暖白、矿物青和靛蓝语义配色，验证图标化入口、资料列表信息层级、704px 对话阅读列、证据轨道、已有会话只读知识库范围以及认证反馈；不验证、不修改任务队列、REST/SSE、数据模型或服务端行为。
- **自动化**：先执行受影响的 Vitest 组件测试，再依次执行 `pnpm lint`、`pnpm prettier:check`、`pnpm typecheck` 与 `pnpm test`；记录实际命令、通过数、失败项及无法执行的原因。
- **手工验收**：在本地登录态分别以 1280px 和 492px 检查登录/注册/退出、知识库创建、资料上传入口与资料详情、发起新对话、已有对话只读范围、流式回答、引用抽屉、账户菜单、键盘焦点和浅/深主题。确认每个图标入口具有可识别名称；确认已有会话不显示知识库切换控件；确认无渐变、霓虹或持续发光等违背既定方向的装饰。
- **记录规则**：仅在 T179 完成后以日期、环境、命令输出摘要、视口和已知限制追加实际结果；计划文本不得被当作已通过的验证记录。

### 阶段 17 Evidence Console UI/UX 初次验收记录（T176–T179，2026-08-26，已由后续修订取代）

- **记录更正**：初次验收只覆盖了知识库行的操作菜单；资料行的阶段/时间元数据与详情/删除菜单、左侧竖向证据轨道、704px 形态断言、注册成功确认及资料空态上传入口尚未完成。不得将本记录作为阶段 17 最终验收依据。
- **自动化结果**：在 `frontend/` 直接使用现有锁定依赖运行 `vitest run`，结果为 `12 passed / 159 passed`；`tsc --noEmit`、`eslint . --max-warnings=0` 与 `prettier --check .` 均通过。通过直接执行本地二进制避免 pnpm 在无交互环境触发模块目录清理；Vitest 输出了既有的配置加载兼容性预告，不影响本次测试结果。
- **手工结果**：在已登录的 `http://localhost:3000`，以 1280px 检查知识库目录、创建入口、资料打开、图标操作菜单和对话空态；以已有会话检查全局历史、新建对话图标、URL 恢复及只读“基于知识库”标签；以 492px 检查固定输入区与导航抽屉。各路径均可用，已有会话未显示知识库切换控件。视口覆盖已恢复默认值。
- **范围确认**：未改动任务队列、REST/SSE、数据模型、错误码、服务端授权、任务状态机或模型出口。

### 阶段 17 历史对话活动账本修订记录（T181，2026-08-26）

- **视觉结果**：全局历史由带外层描边的纸白卡片改为无外框连续条目；低对比行分隔保留列表秩序，当前会话仅以矿物青左侧 2px 标记和极浅背景提示。分页、知识库标签、图标式新建入口及重命名/删除菜单保持原有功能。
- **自动化结果**：`frontend/` 执行 `vitest run`，结果为 `12 passed / 164 passed`；`tsc --noEmit`、`eslint . --max-warnings=0` 与 `prettier --check .` 均通过。Vitest 的既有 Vite 配置加载兼容性预告不影响结果。
- **手工结果**：已登录的 `http://localhost:3000/conversations` 桌面视口复核连续历史列表无白色边框或外层卡片；视口已恢复默认值。

### 阶段 17 Evidence Console UI/UX 最终验收记录（T176–T180，2026-08-26）

- **自动化结果**：在 `frontend/` 使用锁定的本地工具链执行 `vitest run`，结果为 `12 passed / 163 passed`；随后执行 `tsc --noEmit`、`eslint . --max-warnings=0` 与 `prettier --check .`，均通过。Vitest 仍输出既有的 Vite 配置加载兼容性预告，不影响本次结果。
- **回归覆盖**：资料行覆盖 DTO 状态、中文阶段、更新时间及详情/删除次级菜单；空资料列表覆盖上传入口；引用覆盖左侧靛蓝证据轨道与 rank；消息线程覆盖 704px 单列；注册成功覆盖跳转登录页后的明确确认；知识库与对话页覆盖重复信息移除。
- **手工结果**：在已登录的 `http://localhost:3000` 以 1280px 复核单一知识库页头、资料行元数据和图标菜单，以及已有会话仅一处只读“基于知识库”标签；以 492px 复核导航抽屉、知识库选择与固定输入区。视口已恢复默认值。没有对本地资料执行上传或删除等破坏性操作；资料详情/删除、引用轨道和认证成功提示均由上述组件测试验证。
- **范围确认**：未改动任务队列、REST/SSE、数据模型、错误码、服务端授权、任务状态机或模型出口。

## 阶段 11 收尾验证记录（T123/T125，2026-08-19）

快速验证清单（后端优先验证 1–25、前端验证 1–3、质量与交付验证 1–7）的最终执行结果与
任务 T125 要求的全量验证合并记录。执行环境：Windows 11 + Docker Desktop，后端 `uv run`
...
1. `scripts/check-backend.sh` 末步 `verify-contracts.sh` 的相对路径在 `cd backend` 后按新 cwd
   解析而失败（改为脚本自身绝对目录，与 CI 的 `bash ../scripts/verify-contracts.sh` 行为一致）。
2. `tests/integration/test_delivery_stack.py` 完整冒烟的 dockerfile 相对路径依赖 pytest 启动
   cwd（改为仓库根绝对路径），否则 `docker build` 找不到 Dockerfile。
3. 同一冒烟在 api 容器 `--force-recreate` 后立即探测 `/ready` 存在容器启动竞态（uvicorn 尚未
...
浏览器端 API 基地址固定为同源 `/v1`。开发时执行 `pnpm dev` 后，Next.js 仅在开发服务器中将 `/v1/*`
代理至 `ORIONAMESH_API_DEV_UPSTREAM`（默认 `http://127.0.0.1:8000`）；因此本地启动 uvicorn 时无需、也不得为
开发便利而放宽后端 CORS。若本地 API 使用其他地址，只设置此**非公开**变量并重启 Next.js 开发服务器；
`NEXT_PUBLIC_API_BASE_URL` 保持 `/v1`。Compose/生产环境由 Nginx 同源转发 `/v1/*`，不启用此开发代理。
