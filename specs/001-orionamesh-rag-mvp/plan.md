...
- 前端工作区由 `AppShell`、账户菜单和中央内容区组成；本条关于会话历史显示条件的历史结论已由 4.3 取代。当前知识库选择位于对话主内容区顶部，空态输入即新建会话，用户从该控件主动切换知识库时清空当前会话前须确认。当前知识库和会话以 URL 为恢复来源。`ContextRail` 不作为默认布局组成。
...
- 对话首屏未选择知识库时显示明确选择态；知识库选择器以分页、搜索或等价按需加载覆盖全部有效知识库。用户从选择器主动切换知识库后，重置当前会话选中态和引用选择。
